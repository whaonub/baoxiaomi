<template>
  <div class="dt-pages family-reports-detailHistory-page">
    ...
    <div class="current-content" :style="{'top':headerHeight - 2 +'px'}">
                ...
                <div class="xiangxi-item">
                  ...
                  >
                    ...
                    <ul class="zhishu-show">
                      <li v-for="(v,i) in item.secondSecurityList" :key="i">
                        <div class="title fz30">{{v.secondKey}}</div>
                        <div class="range-list">
                          <div class="font-block fz30">{{v.secondValue}}</div>
                          <div class="list">
                            <span>低</span>
                            <span>正常</span>
                            <span>高</span>
                        ...
                        <range class="dt-range" :value="Number(v.secondValue)" disabled></range>
                        <div class="range-list scale">
                          <div class="font-block fz30">{{v.secondValue}}</div>
                          <div class="list">
                            <span>{{v.startbascAmnts}}万</span>
                            <span>{{v.endbascAmnts}}万</span>
                          </div>
                        </div>
                      </li>
                    </ul>
                    <!-- <p class="line-fenge"></p> -->
                  </div>
                </div>
      ...
      <div v-if="!showTip" ref="footer" class="dt-footers dt-footers-animation">
        <div class="guanjia">
          <div class="avatar">
      ...
      >
        <div class="modal-slot-input">
          <span>目标保额(万)</span>
          <input v-model="number1" class="number" type="number">--
          <input v-model="number2" class="number" type="number">
        </div>
      </confirm>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async initData() {
      ...
      this.reportDetailParam.reportId = this.$route.query.reportId;
      // this.reportDetailParam.customerId = this.$route.query.customerId;
      let result = await api.custHistoryList({
        reportId: this.reportDetailParam.reportId
      });
      if (result.status == 1) {
        ...
        if (peoplesIndex != null) {
          ...
          let res = await api.detailReportListHistory({
            shortReportId: this.reportDetailParam.reportId,
    ...
    async changeSwipeData(res) {
      ...
      let securityAdviceData = await api.securityAdviceHistory({
        reportId: this.reportDetailParam.reportId,
...
.family-reports-detailHistory-page {
  .modal-slot-input {
    text-align: center;
    height: 100px;
    line-height: 100px;
                  ...
                  .dt-range {
                    ...
                    .range-bar {
                      ...
                      .range-handle {
                        ...
                        width: 30px;
                        height: 30px;
                        background: url("../../assets/imgs/rande-handle.png")
                          no-repeat center;
