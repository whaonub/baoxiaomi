<template>
  <div class="dt-pages exam-complete-page">
      ...
      <div class="proposal">
        ...
        <div class="proposal-item" v-for="(item,index) in list" :key="index">
          <div class="item-top">
            ...
            <span v-if="item.secondExplain">{{item.secondExplain}}</span>
          </div>
          <div class="item-content">
            <div class="item-target">
              <div class="item-target-li target">
                ...
                <div class="progress">
                  <p class="progressBar">
                    <span class="progressBarValue" :style="{width:(100-((item.targetMin/item.targetMax))*100)+'%'}"></span>
                  </p>
                  <span class="gray start">0万</span>
                  <span class="min" :style="{width:(100-((item.targetMin/item.targetMax)*100))+'%'}">
              ...
              <div class="item-target-li exist">
                ...
                <div class="progress" v-if="item.exist>0">
                  <p class="progressBar" v-if="item.exist>0 && item.exist <= item.targetMax ">
                    <span class="progressBarValue" :style="{width:(item.exist/item.targetMax)*100+'%'}"></span>
                  </p>
                  <p class="progressBar" v-if="item.exist>item.targetMax" :class="item.exist>item.targetMax?'flow':''">
                    <span class="progressBarValue" :style="{width:(item.targetMax/item.exist)*100+'%'}"></span>
                  </p>
                  <span class="gray start">0万</span>
                  <span class="min" v-if="item.exist<item.targetMax" :style="{width:(item.exist/item.targetMax)*100+'%'}">
                    <i>{{item.exist}}万</i>
                  </span>
                  <span class="end">
              ...
              <div class="item-target-li" :class="item.exist==0?'graveNeed':'need'">
                ...
                <div class="progress" v-if="item.exist<item.targetMin && item.exist!=0">
                  <p class="progressBar">
                    <span class="progressBarValue" :style="{width:(((item.targetMax - item.exist)-(item.targetMin -item.exist))/item.targetMax)*100+'%',left:(item.targetMin -item.exist)/item.targetMax*100+'%'}"></span>
                  </p>
                  <span class="gray start">0万</span>
                  <span class="min" :style="{left:(item.targetMin -item.exist)/item.targetMax*100+'%'}">
                    <i>{{item.targetMin-item.exist}}万</i>
                  </span>
                  <span class="section" :style="{width:(item.targetMax - item.exist)/item.targetMax*100+'%'}">
...
export default {
  ...
  methods: {
    async initData() {
      ...
      if (result.status == 1) {
        if (result.data) {
          let list = result.data.secondTypeVos;
